...
export async function updateSemester(id: number, data: Partial<SemesterInput>) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const semester = await tx.semester.update({ where: { id }, data });
    ...
    if (data.costPerCredit !== undefined) {
      const enrollments = await tx.enrollment.findMany({
        where: { status: "ACTIVE", course: { semesterId: id } },
        include: { course: true },
      });
      for (const enrollment of enrollments) {
        const amount =
          Math.round(enrollment.course.credits * data.costPerCredit * 100) /
          100;
        const dueDate = new Date(
          Math.min(
            Math.max(semester.startDate.getTime(), Date.now()),
